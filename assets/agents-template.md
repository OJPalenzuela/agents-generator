...
This template produces the **rich** AGENTS.md. For minimal (agents.md standard), use `agents-minimal-template.md`. For monorepo nested files, use `agents-nested-template.md`.
...
If Claude Code is detected (`.claude/` directory or `CLAUDE.md` exists), also generate a thin `CLAUDE.md` at project root using `assets/claude-template.md`. AGENTS.md is the universal standard — CLAUDE.md only points to it and adds Claude-specific config if any exists.
